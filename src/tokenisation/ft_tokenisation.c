...
/*   Updated: 2024/02/26 07:26:16 by picatrai         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include <minishell.h>

int ft_add_token(t_token **token, char *str)
{
	if (ft_strcmp(str, "<<") == 0)
	{
		if (ft_lst_add_back(token, ft_lstnew(str, WORD, HEREDOC)) == ERROR_MALLOC)
			return (ERROR_MALLOC);
	}
	else if (ft_strcmp(str, ">>") == 0)
	{
		if (ft_lst_add_back(token, ft_lstnew(str, WORD, APPEND)) == ERROR_MALLOC)
			return (ERROR_MALLOC);
	}
	else if (ft_strcmp(str, "<") == 0)
	{
		if (ft_lst_add_back(token, ft_lstnew(str, WORD, INFILE)) == ERROR_MALLOC)
			return (ERROR_MALLOC);
	}
	else if (ft_strcmp(str, ">") == 0)
	{
		if (ft_lst_add_back(token, ft_lstnew(str, WORD, OUTFILE)) == ERROR_MALLOC)
			return (ERROR_MALLOC);
	}
	else if (ft_strcmp(str, "&&") == 0)
	{
		if (ft_lst_add_back(token, ft_lstnew(str, WORD, AND)) == ERROR_MALLOC)
			return (ERROR_MALLOC);
	}
	else if (ft_strcmp(str, "||") == 0)
	{
		if (ft_lst_add_back(token, ft_lstnew(str, WORD, OR)) == ERROR_MALLOC)
			return (ERROR_MALLOC);
	}
	else if (ft_strcmp(str, "|") == 0)
	{
		if (ft_lst_add_back(token, ft_lstnew(str, WORD, PIPE)) == ERROR_MALLOC)
			return (ERROR_MALLOC);
	}
	else if (ft_strcmp(str, "(") == 0)
	{
		if (ft_lst_add_back(token, ft_lstnew(str, WORD, OPEN_PARENTHESIS)) == ERROR_MALLOC)
			return (ERROR_MALLOC);
	}
	else if (ft_strcmp(str, ")") == 0)
	{
		if (ft_lst_add_back(token, ft_lstnew(str, WORD, CLOSE_PARENTHESIS)) == ERROR_MALLOC)
			return (ERROR_MALLOC);
	}
	else
	{
		if (ft_lst_add_back(token, ft_lstnew(str, WORD, TEXT)) == ERROR_MALLOC)
			return (ERROR_MALLOC);
	}
	return (SUCCESS);
}
...
int ft_make_token(t_data_parse *data_parse, t_token **token)
{
	...
	if (data_parse->input[data_parse->index] == '\0')
		return (ERROR_MALLOC);
	while (data_parse->input[data_parse->index] && (data_parse->input[data_parse->index] != ' ' || single_quotes == OPEN || double_quotes == OPEN))
	{
	...
	if (ft_add_token(token, data_parse->str) == ERROR_MALLOC)
		return (ERROR_MALLOC);
	return (SUCCESS);
}
