...
int ft_set_maybe_a_wildcard(char **isol, char ***split, t_wildcard *ls, t_data_parse *data_parse)
{
	...
	*split = ft_split(*isol, '*');
	if (*split == NULL)
		return (free(*isol), free(data_parse->wildcard), ft_free_wildcard(&ls), ERROR_MALLOC);
    return (SUCCESS_SQUARE);
}

int ft_end_maybe_wildcard(int found, t_data_parse *data_parse, char *isol, char **split)
{
    free_2d(split);
	if (found == 0)
    {
...
int	ft_maybe_a_wildcard(t_data_parse *data_parse)
{
    ...
    if (set_ls(&ls) != SUCCESS)
		return (ERROR_MALLOC);
    data_parse->tmp_wildcard = ft_set_maybe_a_wildcard(&isol, &split, ls, data_parse);
    ...
    return(ft_free_wildcard(&ls), ft_end_maybe_wildcard(found, data_parse, isol, split));
}
